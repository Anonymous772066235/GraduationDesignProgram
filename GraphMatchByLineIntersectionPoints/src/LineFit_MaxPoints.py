...
def maxPoints(points, leastpoints=20, mindistance=1e-2):
    ...
    # LINES['B'] = B_max
    # LINES['C'] = C_max
    # LINES.sort_values(by='PN', ascending=False, inplace=True)
    # return LINES
...
def searchLines(points, num_lines, leastpointsOnLine, mindistance):
    i = 0
    Lines = []
    while i < num_lines:
        i += 1
        print('---------------')
        start2=time.time()
        Line = maxPoints(points, leastpointsOnLine, mindistance)
        Lines.append(Line)
        points = delete_points(points, Line, mindistance + 0.2)
        ...
        print('搜索第{:.0f}条直线耗时:{:.0f} hours {:.0f} minutes {:.0f} seconds'.format(i,hours, minutes, seconds))
